...
export default async function OperationsLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-shell"><aside className="sidebar"><div><div className="brand"><span className="brand-mark small">L</span><span>Luma<strong>Resolution Center</strong></span></div><nav><Link href="/operations">Overview</Link><Link href="/operations/cases">Case queue</Link><Link href="/operations/pending">Approvals</Link><Link href="/operations/policies">Policy search</Link></nav></div><div className="account"><span className="avatar">{account.display_name.charAt(0)}</span><div><strong>{account.display_name}</strong><small>Operations</small></div><Logout /></div></aside><main className="main"><header className="topbar"><div><p className="eyebrow">Operations workspace</p><span>Evidence-grounded case resolution</span></div><LiveRefresh /></header>{children}</main></div>;
}
