...
export default async function CasePage({ params }: { params: Promise<{ reference: string }> }) {
  ...
  return <div className="page case-page">
    ...
    <div className="workspace-grid"><div className="workspace-main">
      ...
      <section className="panel detail policy-section"><div className="section-heading"><div><p className="eyebrow">Knowledge</p><h2>Policies applied</h2></div><span className="section-count">{selectedPolicies.size}</span></div>{[...selectedPolicies].map(([sectionId, policy]) => <details className="policy-card" key={sectionId}><summary><div><strong>{String(policy.heading ?? sectionId)}</strong><small>{String(policy.policy_title ?? sectionId)} · {sectionId}</small></div><span>Read policy</span></summary><div className="policy-copy"><p>{String(policy.body || "Policy text is unavailable for this section.")}</p><small>Effective {String(policy.effective_from ?? "for the case event")}{policy.effective_through ? ` through ${String(policy.effective_through)}` : " onward"}</small></div></details>)}{!selectedPolicies.size && <p className="muted">No policy has been applied yet.</p>}</section>
